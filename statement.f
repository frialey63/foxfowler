...
C     "INITIALISE ARRAYS AND SPECIAL VARIABLES"
     ...
     &CSNAME(7), ABVMTH(3), BELMTH(3)
      DIMENSION BELDE(7)
      INTEGER CODENO, SHEETN, BALDT, TRANDT, ACCTNO, F, ABVDT, BELDT
...
C     "FIND CUSTOMER"
   ...
   80 CONTINUE
      GO TO 15
   81 READ(4,7) BRNAME, ANAME, CODENO, SHEETN, BALFWD, BALDT, BALMTH
      DO 82 M=1,7
          I = 4
   ...
   83 WRITE(2,8) BANKYR
      WRITE(2,9) BRNAME, ANAME, CODENO, SHEETN, BALFWD, BALDT, BALMTH
   14 READ(4,11) TRANDT
      WRITE(2,71) TRANDT
...
C     "OUTPUT OF A STATEMENT"
   22 J = 1
      WRITE(6,23)
      WRITE(6,25) CODENO, SHEETN
      WRITE(6,26) ANAME
      WRITE(6,27) BANKYR, BRNAME
      WRITE(6,28)
      WRITE(6,30) BALDT, BALMTH, BALFWD
      TOTAL = BALFWD
...
C     "FIRST TRANSACTION OUTPUT ROUTINE"
   40 IF (DEBS.EQ .-1.0) GO TO 42
      TOTAL = TOTAL - DEBS
      WRITE(6,41) TRANDT, TRANMH, TRANDE, DEBS
      GO TO 58
   42 TOTAL = TOTAL + CREDS
      WRITE(6,43) TRANDT, TRANMH, TRANDE, CREDS
      GO TO 58

C     "SECOND TRANSACTION OUTPUT ROUTINE"
   44 IF (DEBS.EQ .-1.0) GO TO 46
      TOTAL = TOTAL - DEBS
      WRITE(6,45) TRANDT, TRANMH, TRANDE, DEBS, TOTAL
      GO TO 48
   46 TOTAL = TOTAL + CREDS
      WRITE(6,47) TRANDT, TRANMH, TRANDE, CREDS, TOTAL
...
C     "THIRD TRANSACTION OUTPUT ROUTINE"
   49 IF (DEBS.EQ .-1.0) GO TO 51
      TOTAL = TOTAL - DEBS
      WRITE(6,50) TRANDE, DEBS, TOTAL
      GO TO 53
   51 TOTAL = TOTAL + CREDS
      WRITE(6,52) TRANDE, CREDS, TOTAL
...
C     "FOURTH TRANSACTION OUTPUT ROUTINE"
   54 IF (DEBS.EQ .-1.0) GO TO 56
      TOTAL = TOTAL - DEBS
      WRITE(6,55) TRANDE, DEBS
      GO TO 58
   56 TOTAL = TOTAL + CREDS
      WRITE(6,57) TRANDE, CREDS
      GO TO 58
...
C     "END UF STATEMENT OUTPUT"
  ...
  250 WRITE(2,8) BANKYR
   ...
   45 FORMAT(1H , 1X, I2, 1X, 3A1, 1X, 7A4, 1X, F10.2, 12X, F11.2)
